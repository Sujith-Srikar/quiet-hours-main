      ...
      try {
        ...
        if (!response.ok) {
          ...
          try {
            ...
            console.error('Dashboard - Error response (not JSON):', errorText);
            throw new Error(`HTTP ${response.status}: ${errorText || 'Failed to fetch blocks'}`);
          }
          console.error('Dashboard - Error response:', errData);
          throw new Error(errData.message || errData.error || 'Failed to fetch blocks');
        }
        const data = await response.json();
        console.log('Dashboard - Successfully fetched blocks:', data.blocks?.length || 0);
